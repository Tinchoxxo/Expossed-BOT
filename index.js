...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Martin\n' // Nombre
            + 'ORG:Tincho;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=543513155034:+54 351 315 5034\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
/******FIN DE ARCHIVOS ANTILINK POR Tincho******/
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'Expossed-BOT'; if (!author) author = '@tincho_maldonadoo';	
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Bienvenido Tincho!  '))
	})
	...
	client.on('open', () => {
		success('2', 'Conectado Correctamente!')
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				tterro: `Tiene Que Escribir ${prefix}ppt piedra, ${prefix}ppt papel o ${prefix}ppt tijera`,
				waitppt: `⏳Espere un segundo...⏳\n\nEn caso de que no funcione,ingresa de vuelta el comando`,
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa Expossed-bot',
				...
				attp: 'Calma crack estoy haciendo tu texto a sticker 👏\n\n*Esto puede demorar unos minutos*\n\nву Tincho',
					...
					daftarB: `「NO ESTAS REGISTRADO」\n\nPA NO APARECES EN MI BASE DE DATOS ✋🥸🤚\n\nPara poder usarme escribe el siguente comando\n\nComando: ${prefix}daftar Nombre\nEjemplo: ${prefix}daftar Tincho`,
				}
			}
    			const apakah = ['Si','No']
                        const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
			const botNumber = client.user.jid
			const ownerNumber = ["543513155034@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '543513155034@s.whatsapp.net'
...
if (vit == "vitoria") {
var tes = "Vitória del jugador"
}
if (vit == "derrota" ) {
var tes = "Gano el Expossed-BOT"
}
if (vit == "empate" ) {
var tes = "El juego termino en empate"
}
reply(`Expossed-BOT jugo: ${pptb}\nEl jugador jugo: ${args}\n\n${tes}`)
if (tes == "Vitória del jugador") {
reply(pph)
}
break
...
/******JUEGOS SHANDUY LA PUTA MADRE NO TE OLVIDES******/					
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'roleta':
const tiro = ["vacio","vacio","vacio","vacio","vacio","vacio","vacio","vacio","pow","pow"]
...
case 'chance':
              tiringa.updatePresence(from, Presence.composing) 
                ...
                if (args.length < 1) return tiringa.sendMessage(from, `Necesitas escribir correctamente\nEjemplo: ${prefix}chance de que ella me quiera`, text, {quoted: mek})

                random = `${Math.floor(Math.random() * 100)}`
               hasil = `la chance ${body.slice(7)}\n\nes de... ${random}%`
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 MENCIONANDO 〙✪══\n╠➥'+teks+'╚═〘 by Tincho 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
			...
			//ANTILINKS DE REDES SOCIALES	
				...
				//FIN DE ANTILINK HECHO POR Tincho
	...
	//SERVICIO DE MUSICA Y VIDEO 			
		...
		case 'play2':   
	        if (args.length < 1) return reply('Donde esta el nombre de la canción?\n\nEjemplo: *play2 stay with me - 1nonly')
		if (!isUser) return reply(mess.only.daftarB)
	...
	//FIN DE SERVICIO DE MUSICA Y VIDEO			
		...
		if (budy.includes(`Te amo`)) {
                  reply(`Yo Tambien bb,nos casamos?`)
                  }
		...
		if (budy.includes(`Bien gracias y tu?`)) {
                  reply(`callao larva (perdon el que me programo puso esto XD)`)
                  }
	...
	if (budy.startsWith(`goku`)) {
        const none = fs.readFileSync('./mp3/historia.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`soy pobre`)) {
        const none = fs.readFileSync('./mp3/dioses.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`*gime*`)) {
        const none = fs.readFileSync('./mp3/gemido.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`gime`)) {
        const none = fs.readFileSync('./mp3/gemidoo.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`que?`)) {
        const none = fs.readFileSync('./mp3/que.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
